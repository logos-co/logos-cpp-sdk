...
// The half that decides whether a dependency stays optional. `depends` used to
// be the only list, so anything carried there came back out as a REQUIRED
// dependency — auto-loaded, and a load failure when absent.
TEST(LidlGenClient, MetadataJsonKeepsTheTwoDependencyKindsApart)
{
    ModuleDecl m;
    m.name = "two_kinds";
    m.depends = { "hard_dep" };
    m.optional_depends = { "soft_dep" };

    QString json = lidlGenerateMetadataJson(m);
    EXPECT_TRUE(json.contains("\"dependencies\""));
    EXPECT_TRUE(json.contains("\"optional_dependencies\""));

    // Parsed, not grepped: "soft_dep" appears in the text either way, and the
    // whole point is WHICH list it lands in.
    const QJsonObject obj = QJsonDocument::fromJson(json.toUtf8()).object();
    ASSERT_EQ(obj.value("dependencies").toArray().size(), 1);
    EXPECT_EQ(obj.value("dependencies").toArray().at(0).toString(), "hard_dep");
    ASSERT_EQ(obj.value("optional_dependencies").toArray().size(), 1);
    EXPECT_EQ(obj.value("optional_dependencies").toArray().at(0).toString(), "soft_dep");
}

// A module with none emits no key at all, so a metadata.json generated before
// this existed is regenerated byte-for-byte.
TEST(LidlGenClient, MetadataJsonOmitsAnEmptyOptionalList)
{
    ModuleDecl m;
    m.name = "one_kind";
    m.depends = { "hard_dep" };
    ...
    const QJsonObject obj =
        QJsonDocument::fromJson(lidlGenerateMetadataJson(m).toUtf8()).object();
    EXPECT_FALSE(obj.contains("optional_dependencies"));
}
