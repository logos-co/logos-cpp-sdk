#!/usr/bin/env bash
#
# Execute the cpp-sdk module-runtime doc-test end-to-end and regenerate its
# Markdown.
#
...
# against this SDK across its whole closure, building lgpm, packaging/installing
# the accounts module against this SDK, starting the daemon, calling methods)
# and asserts on the output; `doctest generate` renders the same spec to Markdown
# under outputs/; `doctest clean` strips build artifacts so only the generated
# docs remain.
#
...
OUTPUT_DIR="./outputs"
SPEC="cpp-sdk-module-runtime.test.yaml"

# Build the doc-test against THIS repo's current commit rather than the latest
# published flake. The spec overrides `logos-cpp-sdk` across logoscore's closure
# (and the accounts-module build) with `github:logos-co/logos-cpp-sdk{release}`,
# and the pin below makes {release} expand to $COMMIT — so every layer is built
# against exactly what's checked out here. Override by exporting COMMIT (e.g. a
# tag), or set COMMIT="" to fall back to latest master.
#
...
echo "==> Generating ${OUTPUT_DIR}/cpp-sdk-module-runtime.md"
mkdir -p "${OUTPUT_DIR}"
"${DOCTEST[@]}" generate "${SPEC}" \
  ${RELEASE_FOR[@]+"${RELEASE_FOR[@]}"} \
  -o "${OUTPUT_DIR}/cpp-sdk-module-runtime.md"
...
echo "==> Done. Rendered doc is in ${OUTPUT_DIR}/cpp-sdk-module-runtime.md"
